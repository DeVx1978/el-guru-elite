...
export default function LandingPage() {
  ...
  const abrirInfoSeccion = (seccion: string) => {
    ...
    setMenuMovilAbierto(false); // Cerramos hamburguesa para mostrar el texto limpio
  };
  ...
  if (loading || isNavigating) {
    return (
      <div className="splash-master">
        ...
        <style jsx global>{`
          .splash-master { background: radial-gradient(circle at center, #0a0c10 0%, #000 100%) !important; height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; position: fixed; width: 100%; z-index: 999999; }
          .loader-container { position: relative; width: 180px; height: 180px; margin-bottom: 30px; }
          .image-wrapper { width: 100%; height: 100%; border-radius: 50%; overflow: hidden; border: 2px solid #00C853; box-shadow: 0 0 50px rgba(0, 200, 83, 0.4); }
          .image-wrapper img { width: 100%; height: 100%; object-fit: cover; }
          .pulse-ring { position: absolute; top: -10%; left: -10%; width: 120%; height: 120%; border: 1px solid #00C853; border-radius: 50%; animation: pulse-master 2s infinite; opacity: 0.4; }
  ...
  return (
    <div className="elite-landing-master">
      {/* 💉 MODAL COMO PÁGINA INDEPENDIENTE (CORRECCIÓN ESTÉTICA) */}
      {modalAbierto && (
        <div className="info-overlay active">
          <div className="info-page-container">
            <div className="info-header">
              <h3 className="info-title">{modalAbierto.titulo}</h3>
              <button className="info-close" onClick={() => setModalAbierto(null)}><X size={35} color="#00C853" /></button>
            </div>
            <div className="info-body">
              <div className="info-tag"><Zap size={16} color="#00C853" /> INFORMACIÓN DE ÉLITE</div>
      ...
      <nav className="navbar-elite">
        ...
        {/* 💉 HAMBURGUESA SIN EL BOTÓN DUPLICADO ABAJO */}
      ...
      <style jsx global>{`
        ...
        /* 💉 MODAL COMO PÁGINA INDEPENDIENTE (MÓVIL) */
        ...
        .info-page-container { height: 100%; width: 100%; padding: 50px 30px; display: flex; flex-direction: column; box-sizing: border-box; }
        .info-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 50px; }
        .info-title { font-size: 1.8rem; font-weight: 900; text-transform: uppercase; color: white; }
        .info-close { background: none; border: none; cursor: pointer; }
        .info-body { flex-grow: 1; }
        .info-tag { font-size: 12px; font-weight: 900; color: var(--neon); letter-spacing: 4px; margin-bottom: 25px; display: flex; align-items: center; gap: 10px; }
        .info-body p { font-size: 1.2rem; line-height: 1.8; color: #ccc; }
        .info-footer-line { width: 60px; height: 3px; background: var(--neon); box-shadow: 0 0 15px var(--neon); margin-top: 40px; }

        /* 💉 HAMBURGUESA SIN DUPLICADO ABAJO */
        .mobile-menu-overlay { position: fixed; top: 80px; left: 0; width: 100%; height: 0; background: #000; overflow: hidden; transition: 0.5s; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 45px; z-index: 999; }
        ...
        @media (min-width: 1024px) { .hero-elite { padding: 80px 30px; min-height: 85vh; justify-content: center; } }
        ...
        .btn-hero-primary { background: var(--neon); color: black; border: none; padding: 18px 25px; border-radius: 4px; font-weight: 900; flex: 1.5; display: flex; justify-content: center; align-items: center; gap: 10px; font-size: 0.85rem; }
        .btn-hero-secondary { background: transparent; border: 1px solid var(--neon); color: var(--neon); padding: 18px 15px; border-radius: 4px; font-weight: 900; flex: 1; display: flex; justify-content: center; align-items: center; gap: 10px; font-size: 0.8rem; }
        
        .hero-visuals-horizontal { display: flex; flex-direction: column; gap: 15px; margin-top: 50px; width: 100%; }
        @media (min-width: 1024px) { .hero-visuals-horizontal { flex-direction: row; justify-content: space-between; gap: 20px; margin-top: 80px; } .stat-card { flex: 1; } }
