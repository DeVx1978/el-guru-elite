...
const obtenerMetodosPago = (pais: string) => {
  if (pais === 'Ecuador') {
    return [
      ...
      { id: 'western_ec', nombre: 'Western Union', info: 'Beneficiario: Maria José [Apellido] - CI: [ID]' },
      { id: 'nequi_ec', nombre: 'Nequi (Ecuador)', info: 'Transferencia a Celular: +593 [NÚMERO]' },
  ...
  if (pais === 'Colombia') {
    return [
      ...
      { id: 'nequi_co', nombre: 'Nequi (Colombia)', info: 'Celular: 310 123 4567 - Nombre: Maria José' },
      { id: 'usdt_co', nombre: 'USDT (Red TRC20)', info: 'Wallet: TXu4...Red TRC20' },
    ];
  }
  return [
    { id: 'western_global', nombre: 'Western Union (Global)', info: 'Solicitar datos de envío al soporte.' },
...
export default function UnetePage() {
  ...
  const finalizarRegistro = async () => {
    ...
    try {
      ...
      if (errSocio) throw new Error("Correo ya registrado");
  ...
  return (
    <div className="unete-wrapper">
      <nav className="unete-nav">
        <button onClick={() => paso > 1 ? setPaso(paso-1) : router.push('/')} className="nav-back"><ArrowLeft size={18}/> {paso === 5 ? 'INICIO' : 'VOLVER'}</button>
        <div className="progress-bar">{[1, 2, 3, 4].map(p => (<div key={p} className={`dot ${paso >= p ? 'active' : ''}`}></div>))}</div>
      </nav>
        ...
        <div className="form-container">
          {paso === 1 && (
            <div className="fade-in">
              ...
              <div className="input-group">
                <div className="field"><User size={18}/><input name="nombre" placeholder="Nombre Completo" onChange={handleInputChange}/></div>
                <div className="field"><Mail size={18}/><input name="email" placeholder="Email Corporativo" onChange={handleInputChange}/></div>
                <div className="field">
                  <Lock size={18}/><input type={showPass ? "text" : "password"} name="password" placeholder="Contraseña" onChange={handleInputChange}/>
                  <button onClick={() => setShowPass(!showPass)} className="btn-eye">{showPass ? <EyeOff size={18}/> : <Eye size={18}/>}</button>
                </div>
                <div className="field"><Lock size={18}/><input type="password" name="confirmPassword" placeholder="Confirmar Contraseña" onChange={handleInputChange}/></div>
              </div>
              <div className="checks">
                <label className="check-item"><input type="checkbox" name="tyc" onChange={handleInputChange}/><span>Acepto Términos</span></label>
                <label className="check-item"><input type="checkbox" name="politicas" onChange={handleInputChange}/><span>Acepto Privacidad</span></label>
              </div>
          ...
          {paso === 2 && (
            <div className="fade-in">
              ...
              <div className="input-group">
                ...
                <div className="field"><MapPin size={18}/><input name="ciudad" placeholder="Ciudad" onChange={handleInputChange}/></div>
                <div className="phone-grid">
                  <div className="area-code">{formData.codigoArea}</div>
                  <div className="field no-margin"><Phone size={18}/><input name="telefono" placeholder="Número" onChange={handleInputChange}/></div>
                </div>
              </div>
          ...
          {paso === 4 && (
            <div className="fade-in">
              ...
              <h1>VERIFICAR <span>PAGO</span></h1>
              <div className="payment-box">
                <div className="pay-summary">
                  <span>MÉTODO PARA {formData.pais.toUpperCase()}</span>
                ...
                <div className="upload-zone" onClick={() => fileInputRef.current?.click()}>
                  ...
                  {comprobanteUrl ? <img src={comprobanteUrl} className="preview"/> : <><UploadCloud size={32}/><p>Adjuntar Pago</p></>}
                </div>
              </div>
              <button disabled={loading || !comprobante} onClick={finalizarRegistro} className="btn-primary">{loading ? 'PROCESANDO...' : 'FINALIZAR REGISTRO'}</button>
            </div>
          )}
          ...
          {paso === 5 && (
            <div className="fade-in success-screen">
              ...
              <p>Un administrador validará su pago en las próximas horas.</p>
      ...
      <style jsx global>{`
        ...
        .upload-zone { border: 2px dashed #111; border-radius: 15px; height: 150px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444; font-size: 12px; font-weight: 800; cursor: pointer; overflow: hidden; }
