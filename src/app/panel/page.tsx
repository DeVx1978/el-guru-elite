...
function SocioPanel() {
    ...
    const [isClient, setIsClient] = useState(false); // ← Control de hidratación
    ...
    useEffect(() => {
        ...
        // Carga unificada desde tabla 'socios'
        conectarBovedaUnificada(socioId);
    }, [isClient, router]);

    // ==================== FUNCIÓN CORREGIDA (UNIFICADA) ====================
    const conectarBovedaUnificada = async (idSocio: string) => {
        try {
            const { data: socio, error } = await clientSupabase
                .from('socios')
                .select('*')
                .eq('id', idSocio)
                .single();
            ...
            // Todo se lee ahora de la tabla socios (inversion, utilidad_total, nivel_socio, etc.)
            const inversion = Number(socio.inversion) || Number(socio.inversion_minima) || 0;
            const ganancia = Number(socio.utilidad_total) || 0;

            setBalance(inversion + ganancia);
            setUtilidad(ganancia);
            setNivelSocio(socio.nivel_socio || "Socio Élite");
            setEditPais(socio.pais || "Colombia");
            setEditTelefono(socio.telefono || "");
            setEditCiudad(socio.ciudad || "");
            setEditEmail(socio.email || "");
            setEsAdmin(socio.rol === 'admin' || socio.email === 'mariajose@gmail.com');

        } catch (err) {
            console.error("Error inesperado en bóveda:", err);
    ...
    const actualizarPerfil = async () => {
        ...
        const socioId = localStorage.getItem('socio_id');
        try {
            await clientSupabase.from('socios').update({
                nombre: editNombre,
                pais: editPais,
    ...
    return (
        <div className="premium-mansion-layout">
            ...
            <div className="premium-viewport">
                ...
                {/* BOTTOM NAV MOBILE - SIN DUPLICADOS */}
